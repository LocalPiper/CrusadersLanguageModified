...
struct IRInstruction {
  ...
  IRInstruction(IROpcode op_, std::string res = "", std::string a1 = "", std::string a2 = "", std::string opStr = "")
        : opcode(op_), result(std::move(res)), arg1(std::move(a1)), arg2(std::move(a2)), op(std::move(opStr)) {}
};
  ...
  for (const auto& instr : ir) {
    switch (instr.opcode) {
      ...
      case IROpcode::BINARY_OP:
        std::cout << instr.result << " = " << instr.arg1 << " " << instr.op << " " << instr.arg2 << "\n";
        break;
